          ...
          <Accordion type="single" collapsible className="flex flex-col gap-2 md:gap-4">
            {faqItems.map((item, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="border border-gray-200 rounded-lg px-6 data-[state=open]:border-primary transition-colors"
              >
                <AccordionTrigger className="text-left text-[#1E2842] font-semibold text-sm md:text-base py-5 hover:text-primary transition-colors">
                  {item.question}
                </AccordionTrigger>
